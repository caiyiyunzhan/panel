...
export default ({ database, className }: Props) => {
    ...
    const schema = object().shape({
        confirm: string()
            .required(t("database_name_required"))
            .oneOf([database.name.split('_', 2)[1], database.name], t("database_name_match")),
    });
    ...
    return (
        <>
            <Formik onSubmit={submit} initialValues={{ confirm: '' }} validationSchema={schema} isInitialValid={false}>
                {({ isSubmitting, isValid, resetForm }) => (
                    ...
                    >
                        ...
                        <p css={tw`text-sm`}>
                            {t("database_deletion_message", { name: database.name })}
                        </p>
                        <Form css={tw`m-0 mt-6`}>
                            <Field
                                ...
                                description={t("confirm_database_name_desc")}
                            />
            ...
            <Modal visible={connectionVisible} onDismissed={() => setConnectionVisible(false)}>
                ...
                <h3 css={tw`mb-6 text-2xl`}>{t("database_connection_details")}</h3>
                <div>
                ...
                <div css={tw`mt-6`}>
                    <Label>{t("connections_from")}</Label>
                ...
                <div css={tw`mt-6`}>
                    <Label>{t("jdbc_connection_string")}</Label>
            ...
            <GreyRowBox $hoverable={false} className={className} css={tw`mb-2`}>
                ...
                <div css={tw`ml-8 text-center hidden md:block`}>
                    ...
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>{t("endpoint")}</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <p css={tw`text-sm`}>{database.allowConnectionsFrom}</p>
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>{t("connections_from")}</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <CopyOnClick text={database.username}>
                        <p css={tw`text-sm`}>{database.username}</p>
                    </CopyOnClick>
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>{t("username")}</p>
                </div>
                <div css={tw`ml-8`}>
