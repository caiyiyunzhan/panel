...
export default () => {
    ...
    return (
        <PageContentBlock title={t("ssh_keys")}>
            <FlashMessageRender byKey={"account"} />
            <div css={tw`md:flex flex-nowrap my-10`}>
                <ContentBox title={t("add_ssh_key")} css={tw`flex-none w-full md:w-1/2`}>
                    <CreateSSHKeyForm />
                </ContentBox>
                <ContentBox title={t("ssh_keys")} css={tw`flex-1 overflow-hidden mt-8 md:mt-0 md:ml-8`}>
                    <SpinnerOverlay visible={!data && isValidating} />
                    {!data || !data.length ? (
                        <p css={tw`text-center text-sm`}>
                            {!data ? t("loading") : t("no_ssh_keys")}
                        </p>
                    ) : (
                        data.map((key, index) => (
                            <GreyRowBox
